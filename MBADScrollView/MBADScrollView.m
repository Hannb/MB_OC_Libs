...
#define pageSize (myHeight * 0.15 > 25 ? 25 : myHeight * 0.15)

#import "MBADScrollView.h"
#import "MBWebImageManager.h"
//#import "UIPageControl+Radius.h"

@interface MBADScrollView () <UIScrollViewDelegate>

@property (nonatomic,copy) NSArray *imageData;

@end

@implementation MBADScrollView {
...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[MBADScrollView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (void)preparePageControl {
    ...
    //65,164,250
    page.currentPageIndicatorTintColor =  [UIColor colorWithRed:65/255.0 green:164/255.0 blue:250/255.0 alpha:1];//[UIColor blueColor];
    page.numberOfPages = _MaxImageCount;
    page.currentPage = 0;
    
//    [page setValue:[UIImage imageNamed:@"selected_roll"] forKey:@"_currentPageImage"];
//    [page setValue:[UIImage imageNamed:@"nomal_roll"] forKey:@"_pageImage"];
...
- (void)setStyle:(PageControlStyle)style {
    if (style == PageControlAtRight) {
        ...
        _PageControl.center = CGPointMake(myWidth-w*0.5 -12, myHeight-pageSize * 0.5);
    }
}
...
- (UIImage *)setImageWithIndex:(NSInteger)index {
    ...
    //从内存缓存中取,如果没有使用占位图片
    UIImage *image = [[[MBWebImageManager shareManager] webImageCache] valueForKey:_imageData[index]];
...
- (void)getImage {
    ...
    for (NSString *urlSting in _imageData) {
        [[MBWebImageManager shareManager] downloadImageWithUrlString:urlSting];
    }
